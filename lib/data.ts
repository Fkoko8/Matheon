...
export async function getReviewQueue(userId?: string): Promise<Question[]> { const supabase = createClient(); const uid = userId ?? await getUserId(); if (!supabase || !uid) return []; const { getReviewQueueForToday } = await import('@/lib/learning/review'); const dueItems = await getReviewQueueForToday(uid, supabase); const questions = await Promise.all(dueItems.map((item) => getQuestion(item.questionId))); return questions.filter((question): question is Question => Boolean(question)) }
...
export async function getDashboardData(userId?: string): Promise<DashboardData> { const [profile, progress, review, topicsList] = await Promise.all([getProfile(), getUserProgress(userId), getReviewQueue(userId), getTopics()]); const mastery = progress.length ? Math.round(progress.reduce((sum, item) => sum + item.mastery, 0) / progress.length) : 0; const accuracy = progress.length ? Math.round(progress.reduce((sum, item) => sum + item.accuracy, 0) / progress.length) : 0; return { profile, mastery, accuracy, tasks: review.length, reviewQueue: review.length, weakTopics: topicsList.filter((topic) => (progress.find((item) => item.topicId === topic.id)?.mastery ?? 0) < 60), recentActivity: [] } }
